...
-- immediately.
tryAwait :: Monad m => Pipe l a b m (Maybe a)
...
groupBy :: Monad m => (a -> a -> Bool) -> Pipe l a [a] m r
...
filter :: Monad m => (a -> Bool) -> Pipe l a a m r
filter p = forever $ takeWhile_ p

-- | Feed an input element to a pipe.
feed :: Monad m => a -> Pipe Void a b m r -> Pipe Void a b m r
